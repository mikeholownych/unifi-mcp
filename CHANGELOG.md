...
### Fixed
- `update_site_settings` now writes to `set/setting/{key}` instead of PUT
  `rest/setting`, which controllers reject with HTTP 500. The payload must name
  the target section via a `"key"` field; a missing key now raises a clear error
